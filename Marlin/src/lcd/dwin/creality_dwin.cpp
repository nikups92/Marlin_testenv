    ...
    void Draw_Bed_Mesh(int16_t selected = -1, uint8_t gridline_width = 1, uint16_t padding_x = 8, uint16_t padding_y_top = 40 + 53 - 7) {
      ...
      GRID_LOOP(x, y) {
        ...
        while (LCD_SERIAL.availableForWrite() < 32) { // wait for serial to be available without blocking and resetting the MCU 
          gcode.process_subcommands_now((char*)"G4 P10");
          planner.synchronize();
        } 
        // Draw value text on 
        if (viewer_print_value) { 
          gcode.process_subcommands_now((char*)"G4 P10");  // still fails without additional delay...
          planner.synchronize();
      ...
      switch(item) {
        ...
        case HOME_X:
          if (draw) {
            ...
            Popup_Handler(Home);
            gcode.process_subcommands_now((char*)("G28 X"));
            planner.synchronize();
            Redraw_Menu();
          }
          break;
        case HOME_Y:
          if (draw) {
            ...
            Popup_Handler(Home);
            gcode.process_subcommands_now((char*)("G28 Y"));
            planner.synchronize();
            Redraw_Menu();
          }
          break;
        case HOME_Z:
          if (draw) {
            ...
            Popup_Handler(Home);
            gcode.process_subcommands_now((char*)("G28 Z"));
            planner.synchronize();
            Redraw_Menu();
          }
          break;
        case HOME_SET:
          if (draw) {
            ...
            gcode.process_subcommands_now((char*)("G92 X0 Y0 Z0"));
            AudioFeedback();
          }
          break;
      }
          ...
          case ZOFFSET_HOME:
            if (draw) {
              ...
              Popup_Handler(MoveWait);
                ...
                gcode.process_subcommands_now(cmd);
              #else
                gcode.process_subcommands_now((char*)("G0 F4000 X117.5 Y117.5"));
              #endif
              gcode.process_subcommands_now((char*)("G0 F300 Z0"));
              planner.synchronize();
              Redraw_Menu();
            }
            break;
          ...
          case ZOFFSET_UP:
            if (draw) {
              ...
              if (zoffsetvalue < MAX_Z_OFFSET) {
                if(liveadjust) {
                  gcode.process_subcommands_now((char*)("M290 Z0.01"));
                  planner.synchronize();
                }
                zoffsetvalue += 0.01;
          ...
          case ZOFFSET_DOWN:
            if (draw) {
              ...
              if (zoffsetvalue > MIN_Z_OFFSET) {
                if(liveadjust) {
                  gcode.process_subcommands_now((char*)("M290 Z-0.01"));
                  planner.synchronize();
                }
                zoffsetvalue -= 0.01;
          ...
          case CHANGEFIL_LOAD:
            if (draw) {
              ...
              if (thermalManager.temp_hotend[0].target < thermalManager.extrude_min_temp) {
                ...
                gcode.process_subcommands_now((char*)("M701"));
                planner.synchronize();
                Redraw_Menu();
              }
            }
          ...
          case CHANGEFIL_UNLOAD:
            if (draw) {
              ...
              if (thermalManager.temp_hotend[0].target < thermalManager.extrude_min_temp) {
                ...
                gcode.process_subcommands_now((char*)("M702"));
                planner.synchronize();
                Redraw_Menu();
              }
            }
        ...
        switch (item) {
          ...
          case LEVELING_GET_MESH:
            if (draw) {
              ...
              gcode.home_all_axes(true);
                ...
                #if HAS_BED_PROBE
                  Popup_Handler(Level);
                  gcode.process_subcommands_now((char*)("G29 P1"));
                  gcode.process_subcommands_now((char*)("G29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nG29 P3\nM420 S1"));
                  planner.synchronize();
              ...
              #elif HAS_BED_PROBE
                Popup_Handler(Level);
                gcode.process_subcommands_now((char*)("G29"));
                planner.synchronize();
                Popup_Handler(SaveLevel);
              #else
                level_state = planner.leveling_active;
          ...
          case LEVELING_LOAD:
            if (draw) {
              ...
              gcode.process_subcommands_now((char*)("G29 L"));
              planner.synchronize();
              AudioFeedback(true);
            }
            break;
          case LEVELING_SAVE:
            if(draw) {
              ...
              gcode.process_subcommands_now((char*)("G29 S"));
              planner.synchronize();
              AudioFeedback(true);
            }
            break;
          ...
          case LEVELING_SETTINGS_FADE:
            ...
            case LEVELING_SETTINGS_PLANE:
                if (draw) {
                  ...
                  gcode.process_subcommands_now((char*)("M420 S1"));
                  planner.synchronize();
                  AudioFeedback(true);
                }
                break;
        ...
        switch (item) {
          case MMESH_BACK:
            if (draw) {
              ...
              gcode.process_subcommands_now((char*)("G29 A"));
              planner.synchronize();
              set_bed_leveling_enabled(level_state);
              Draw_Menu(Leveling, LEVELING_GET_MESH);
            }
        ...
        if (paused) {
          if (sdprint) {
            wait_for_user = false;
              ...
              gcode.process_subcommands_now((char*)(cmnd));
